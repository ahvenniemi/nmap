...
local output_file = "/path/to/desired/folder/sX_results.txt"  -- Output file name

local ports = "1-1000"  -- Port range to scan for TCP Xmas scan

-- Perform the scan
local success, result = pcall(nmap.scan, {target}, ports, "-sX")
...
if success then
    ...
    print("TCP Xmas scan results saved to " .. output_file)
else
    -- Print an error message if the scan failed
    print("TCP Xmas scan failed: " .. result)
end
