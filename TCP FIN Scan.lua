...
local output_file = "/path/to/desired/folder/sF_results.txt"  -- Output file name

local ports = "1-1000"  -- Port range to scan for TCP FIN scan

-- Perform the scan
local success, result = pcall(nmap.scan, {target}, ports, "-sF")
...
if success then
    ...
    print("TCP FIN scan results saved to " .. output_file)
else
    -- Print an error message if the scan failed
    print("TCP FIN scan failed: " .. result)
end
