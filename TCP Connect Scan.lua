...
local output_file = "/path/to/desired/folder/sT_results.txt"  -- Output file name

local ports = "1-1000"  -- Port range to scan for TCP connect scan

-- Perform the scan
local success, result = pcall(nmap.scan, {target}, ports, "-sT")
...
if success then
    ...
    print("TCP connect scan results saved to " .. output_file)
else
    -- Print an error message if the scan failed
    print("TCP connect scan failed: " .. result)
end
