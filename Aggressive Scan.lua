...
local output_file = "/path/to/desired/folder/aggressive_scan_results.txt"  -- Output file name

-- Perform the scan
local success, result = pcall(nmap.scan, {target}, "-A")
...
if success then
    ...
    print("Aggressive scan results saved to " .. output_file)
else
    -- Print an error message if the scan failed
    print("Aggressive scan failed: " .. result)
end
