...
local output_file = "/path/to/desired/folder/sS_results.txt"  -- Output file name

local ports = "1-1000"  -- Port range to scan for TCP SYN scan

-- Perform the scan
local success, result = pcall(nmap.scan, {target}, ports, "-sS")
...
if success then
    ...
    print("TCP SYN scan results saved to " .. output_file)
else
    -- Print an error message if the scan failed
    print("TCP SYN scan failed: " .. result)
end
