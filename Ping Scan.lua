...
local output_file = "/path/to/desired/folder/sP_results.txt"  -- Output file name

-- Perform the scan
local success, result = pcall(nmap.scan, {target}, "-sn")
...
if success then
    ...
    for _, host in ipairs(result) do
        local line = "IP: " .. host.ip .. " Status: " .. host.status.state .. "\n"
        file:write(line)
    end
    ...
    print("Ping scan results saved to " .. output_file)
else
    -- Print an error message if the scan failed
    print("Ping scan failed: " .. result)
end
