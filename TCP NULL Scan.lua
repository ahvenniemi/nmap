...
local output_file = "/path/to/desired/folder/sN_results.txt"  -- Output file name

local ports = "1-1000"  -- Port range to scan for TCP NULL scan

-- Perform the scan
local success, result = pcall(nmap.scan, {target}, ports, "-sN")
...
if success then
    ...
    print("TCP NULL scan results saved to " .. output_file)
else
    -- Print an error message if the scan failed
    print("TCP NULL scan failed: " .. result)
end
